...
func (p *pgstatCollector) name() string {
	return "pg_stat_database"
}

func (p *pgstatCollector) Update(ch chan<- prometheus.Metric) error {
	queryInstance, ok := config.MetricMap[p.name()]
	if !ok {
		return fmt.Errorf("can not find pg_stat_database from MetricMap")
	}
...
func init() {
	collector.RegisterCollector("pg_stat_database", collector.DefaultEnabled, NewStatCollector)
}

func NewStatCollector(logger log.Logger) (collector.Collector, error) {
