...
func (p *pgStatActivityCollector) name() string {
	return "pg_stat_activity"
}

func (p *pgStatActivityCollector) Update(ch chan<- prometheus.Metric) error {
	queryInstance, ok := config.MetricMap[p.name()]
	if !ok {
		return fmt.Errorf("can not find pg_stat_activity from MetricMap")
	}
...
func init() {
	collector.RegisterCollector("pg_stat_activity", collector.DefaultEnabled, NewpgStatActivityCollector)
}

func NewpgStatActivityCollector(logger log.Logger) (collector.Collector, error) {
